...
async function main() {
  ...
  const rows = await db.select().from(purchaseRequests)
    ...
    .limit(1);
  const req = rows[0];
  console.log("Status atual:", req?.status, "| urgencyLevel:", req?.urgencyLevel, "| id:", req?.id);

  if (req) {
    const history = await db.select().from(approvalHistory)
      .where(eq(approvalHistory.requestId, req.id));
    console.log("\nHistórico de aprovações:");
    console.table(history.map(h => ({ step: h.step, action: h.action, user: h.userName, date: h.createdAt })));
  }
